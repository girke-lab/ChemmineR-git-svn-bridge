...
\noindent Several methods and functions are available to compute basic compound descriptors, such as molecular formula (MF), molecular weigth (MW) and atom frequencies. In all these function, it is important to set \Rfunarg{addH=TRUE} in order to include/add hydrogens that are often not specified in an SD file.
...
propma <- data.frame(MF=MF(sdfset, addH=FALSE), MW=MW(sdfset, addH=FALSE), 
                     atomcountMA(sdfset, addH=FALSE)); propma[1:4,]
...
.
@
\noindent The function \Rfunction{bonds} provides related information. If \Rfunarg{type="bonds"}, a data frame is returned with bonds and charges, if \Rfunarg{type="charge"}, all charged atoms are returned and if \Rfunarg{type="addNH"} the number of missing hydrogens are returned for each molecule. 
<<eval=TRUE, keep.source=TRUE>>=
