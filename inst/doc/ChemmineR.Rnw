...
\markboth{Molecular Property Functions}{} % Only required to print section title in header field without numbering. 
...
<<boxplot, eval=FALSE, fig=TRUE, keep.source=TRUE>>=
...
\section{Connection Tables}
\markboth{Connection Tables}{} % Only required to print section title in header field without numbering. 
\noindent Connection tables of compounds provide an efficient data structure for many basic computations on small molecules. The function \Rfunction{conMA} creates this data structure from \Rclass{SDF} and \Rclass{SDFset} objects. The resulting connection matrix contains the atom labels in the row/column titles and the bond types in the data part. The labels are defined as follows: 0 is no conntection, 1 is a single bond, 2 is a double bond and 3 is a triple bond.
<<contable, eval=FALSE, fig=FALSE, keep.source=TRUE>>=
conMA(sdfset[1:2], exclude=c("H"))
   # Create connection matrix for first two molecules in sdfset
conMA(sdfset[[1]], exclude=c("H"))
   # Return connection matrix for first molecule
...
\noindent For more details, consult help documentation with \Rfunction{?plotStruc} or \Rfunction{?plot}.
