...
The output is a cluster \Rclass{vector} with the item labels in the name slot and the cluster IDs in the data slot. As third parameter the user can set a
minimum similarity \Rfunarg{cutoff} value for generating the nearest neighbor table. The latter is an optional setting that is not part of the original Jarvis-Patrick algorithm.\\
...
<<eval=TRUE, keep.source=TRUE>>=
jarvisPatrick(nearestNeighbors(fpset,cutoff=0.6, method="Tanimoto"), k=2 ,mode="b")
@
