...
\section{OpenBabel Accelerated Functions}
ChemmineR is now able to take advantage of accelerated funcationality implmented in the OpenBabel C++ library. This functionality
can be enabled by installing the ChemmineOB package. ChemmineR will automatically detect it's presence and use the faster version of a
function, or enable additinal functionality.  This section will be kept up to date with a list of things that take advantage of OpenBabel.

Format conversions between SMILES and SDF formats can now be done quickly for multiple compounds. See Section \ref{sec:conversions} for details.
...
\noindent When the ChemineOB package is installed these conversions can 
be done quicly using the OpenBabel library. Otherwise these functions
will fall back to using the ChemMine Tools web service for conversion
with the Open Babel Open Source Chemistry Toolbox. They will then
require internet connectivity and will be limited to only the first compound given.
