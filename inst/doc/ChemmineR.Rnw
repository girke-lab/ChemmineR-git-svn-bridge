...
<<eval=TRUE, keep.source=TRUE>>=
cmp.search(apset, apset[1], type=1, cutoff = 0.3, quiet=TRUE) 
cmp.search(apset, apset[1], type=2, cutoff = 0.3, quiet=TRUE) 
@
...
plot(sdfset[names(cmp.search(apset, apset[6], type=2, cutoff=4, quiet=TRUE))], 
     print=FALSE)
@
...
identify very similar compounds in compound sets, which will be frequently, but
...
binning clustering function \Rfunction{cmp.cluster}. The function requires as input a
