...
\Rpackage{ChemmineR} is an R package for analyzing small molecule and screening data. It contains functions for processing SDFs (structure data files), structural similarity searching, clustering/diversity analyses of compound libraries with a wide spectrum of algorithms and utilities for managing complex data sets from high-throughput compound bio-assays. 

\begin{figure}[bthp]
  \centering
   \includegraphics[width=\textwidth]{overview.png}
   \caption{Overview of functions provided by the \Rpackage{ChemmineR}
   package.}
   \label{fig:overview}
\end{figure}
...
\subsection{One Minute Tutorial}
The following code gives an overview of the most important functionalities provided by \Rpackage{ChemmineR}. Copy and paste of the commands into the R console will demonstrate these utilities. \\
...
\noindent This object class is created during the import of an SD file:
<<eval=FALSE, keep.source=TRUE>>=
...
\noindent Converting the data blocks in \Rclass{SDFset} to a matrix:
<<eval=FALSE, keep.source=TRUE>>=
...
cmp.cluster(db=apset, cutoff = c(0.65, 0.5))[1:4,] 
...
\section{Molecular Property Functions}
\noindent Several methods and functions are available to compute basic compound descriptors, such as molecular formula (MF), molecular weigth (MW) and atom frequencies. 
...
\noindent The following shows an example for asssigning the values stored in a matrix (\textit{e.g.} property descriptors) to the data block components in an \Rclass{SDFset}. Each matrix row will be assigned to the corresponding slot position in the \Rclass{SDFset}.
<<eval=TRUE, keep.source=TRUE>>=
datablock(sdfset) <- propma
datablock(sdfset)[1:4]
@
...
\subsection{Pairwise Compound Comparisons}
\noindent The \Rfunction{cmp.similarity} function computes the atom pair similarity between two compounds using the Tanimoto coefficient as similarity measure. 
<<eval=TRUE, keep.source=TRUE>>=
...
\section{Clustering Compound Structures}
\markboth{Clustering Compound Structures}{} % Only required to print section title in header field without numbering. 
<<eval=FALSE, keep.source=TRUE>>=
