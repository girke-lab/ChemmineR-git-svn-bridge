...
\begin{itemize}
	\item Integration of a subset of OpenBabel functionalities via new \Rpackage{ChemmineOB} add-on package
...
functionalities implemented in the OpenBabel C++ library. These
...
\noindent When the \Rpackage{ChemineOB} package is installed these conversions can 
be done quickly using the OpenBabel library. Otherwise these functions
will fall back to using the ChemMine Tools web service for conversion
with the OpenBabel Open Source Chemistry Toolbox. They will then
...
string to an SDF string, one can use the the \Rfunction{convertFormat}
function.
