...
\noindent Compute atom pair fingerprint matrix using internal atom pair selection containing the 4096 most common atom pairs identified in DrugBank's compound collection. For details see \Rclass{?apfp}. The following example uses from this set the 1024 most frequent atom pairs:
<<eval=FALSE, keep.source=TRUE>>=
apfpset <- desc2fp(x=apset, descnames=1024, type="matrix")
@

\noindent Alternatively, one can provide any custom atom pair selection. Here, the 1024 most common ones in apset object: 
<<eval=FALSE, keep.source=TRUE>>=
...
\noindent Converting character fingerprints back to a matrix:
<<eval=FALSE, keep.source=TRUE>>=
