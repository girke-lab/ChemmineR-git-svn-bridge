...
\Rpackage{ChemmineR} is a cheminformatics package for analyzing drug-like small molecule data in R. Its new version ChemmineR-V2 contains functions for processing SDFs (structure data files), compound depictions, structural similarity searching, property predictions, clustering/diversity analyses of compound libraries with a wide spectrum of algorithms. 
...
\noindent Read atom pair string representation from file into \Robject{APset}: 
<<eval=FALSE, keep.source=TRUE>>=
apset <- read.AP(x="matrix.xls", colid="AP")
...
Atom pairs can be converted into binary atom pair fingerprints of fixed length. Computations on this more compact data structure are more time and memory efficient than on their more complex atom pair counterparts. The function \Rfunction{desc2fp} generates fingerprints from descriptor vectors of variable length such as atom pairs stored in \Rclass{APset} or \Rclass{list} containers. The obtained fingerprints can be used for structure similarity comparisons, searching and clustering. \\ 
...
\noindent Compute atom pair fingerprint matrix using internal atom pair selection containing 4096 most common atom pairs in DrugBank. For details see \Rclass{?apfp}. The following example uses from this set the 1024 most frequent atom pairs:
<<eval=FALSE, keep.source=TRUE>>=
apfpset <- desc2fp(x=apset, descnames=1024, type="matrix")
@

\noindent Alternatively, one can provide any custom atom pair selection. Here 1024 most common ones in apset object: 
<<eval=FALSE, keep.source=TRUE>>=
...
<<eval=FALSE, keep.source=TRUE>>=
apfpchar <- desc2fp(x=apset, descnames=1024, type="character")
@

\noindent Converting character fingerprints back to matrix:
<<eval=FALSE, keep.source=TRUE>>=
apfpma <- matrix(as.numeric(unlist(strsplit(apfpchar, ""))), length(apfpchar),
                 nchar(apfpchar[1]), byrow=TRUE, dimnames=list(names(apfpchar),
                 1:nchar(apfpchar[1])))
@

\noindent Similarity searching returning Tanimoto similarity coefficients:
<<eval=FALSE, keep.source=TRUE>>=
fpSim(x=apfpset[1,], y=apfpset)
@

\noindent Clustering example:
<<eval=FALSE, keep.source=TRUE>>=
simMAap <- sapply(rownames(apfpset), function(x) fpSim(x=apfpset[x,], apfpset, sorted=FALSE))
...
simMA <- sapply(rownames(fpset), function(x) fpSim(x=fpset[x,], fpset, sorted=FALSE)) 
