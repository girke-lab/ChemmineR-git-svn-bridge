...
*Structure format interconversions* (see Section [sec:conversions])
...
-   *propOB*: generates several compound properites
...
```{r eval=FALSE, tidy=FALSE}
 sdfStr <-
 convertFormat("SMI","SDF","CC(=O)OC1=CC=CC=C1C(=O)O_name") 
```
...
retrieval. This is still an experimental feature. The default database
...
The function *loadSdf* can be used to load
*SDF* data, either from a file or *SDFset*
object. The *fct* parameter should be a function to
...
with data from and *SDFset*. We also define
...
```{r eval=TRUE, tidy=FALSE}
 lightIds <-
 findCompounds(conn,"MW",c("MW < 300")) 
