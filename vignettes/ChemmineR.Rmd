...
`r message("mark a"); citet(c("Cao_2008"))` . Since then many additional
...
`r message("mark b"); citep(c("Backman_2011","Wang_2013"))`).
...
-   Integration of a subset of OpenBabel functionalities via new
    *ChemmineOB* add-on package `r citep(c("Cao_2008"))`
...
`r citep(c("greycite13432", "Cao_2008" ))` . These
...
ring perception algorithm from `r citet(c("Hanser_1996"))` . In addition, the function can return
...
`r citet(c("Wang_2013"))`. The following gives a
...
`r citep(c("Carhart_1985","Chen_2002"))` . It returns a searchable atom pair database
...
cheminformatics `r citep("greycite13371")`. It requires a nearest neighbor table, which consists
...
```{r listCMTools, eval=TRUE, tidy=FALSE}
listCMTools()
```
...
```{r toolDetailsCMT, eval=TRUE, tidy=FALSE}
